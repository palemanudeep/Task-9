...
resource "aws_ecs_task_definition" "medusa_postgres" {
  ...
  cpu                      = "512"
  memory                   = "1024"
  execution_role_arn       = aws_iam_role.role_for_the_ecs_tasks_task8.arn
  task_role_arn            = aws_iam_role.role_for_the_ecs_tasks_task8.arn
...
resource "aws_ecs_service" "postgres_service" {
  name                   = "medusa-postgres-service"
  cluster                = aws_ecs_cluster.cluster_to_deploy_the_containers_task8.id
...
resource "aws_ecs_service" "task8_medusa" {
  name                   = "task8_medusa-service"
  cluster                = aws_ecs_cluster.cluster_to_deploy_the_containers_task8.id
